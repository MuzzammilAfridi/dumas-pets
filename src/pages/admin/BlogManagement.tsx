...
const BlogManagement = () => {
  ...
  const save = async (asDraft: boolean) => {
    ...
    try {
      if (editing) {
        await blogService.updatePost({ ...payload, name: editing.name });
  ...
  const handleDelete = async () => {
    ...
    await blogService.deletePost(confirmDelete.name);
              ...
              <Section title="Additional Settings">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <ToggleRow
                    ...
                    checked={form.email_notification === 1}
                    onChange={(v) =>
                      update("email_notification", v ? 1 : 0)
                    }
                  />
                  <ToggleRow
