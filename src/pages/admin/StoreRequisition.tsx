...
const handleExport = async (label: string) => {
  if (label === 'Excel') {
    ...
    const csv = [
      ...
      ...summary.map(s =>
        [
          ...
          s.available,
          s.shortage,
          s.isShort ? 'Low Stock' : 'Sufficient',
        ].join(',')
      ),
    ].join('\n');
    ...
    a.href = url;
    a.download = `requisition-${Date.now()}.csv`;
    a.click();
  ...
  const html = `
    <div style="padding:20px;font-family:Arial;">
      <h1 style="margin-bottom:5px;">Store Requisition</h1>
      <p style="margin-bottom:20px;">
        Generated from ${mockOrders.length} orders
      </p>
  ...
  html2pdf()
    ...
    .set({
      margin: 10,
      filename: `store-requisition-${Date.now()}.pdf`,
